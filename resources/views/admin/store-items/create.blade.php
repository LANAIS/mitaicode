...
@section('title', 'Crear Nuevo Item - Panel de Administración')

@section('content')
<div class="container-fluid">
    <!-- Encabezado -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Crear Nuevo Item</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('admin.store-items.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">Recomendado: 512x512px, formato PNG con fondo transparente.</small>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control @error('category') is-invalid @enderror" id="category" name="category" required>
                                    ...
                                    <option value="avatar" {{ old('category') == 'avatar' ? 'selected' : '' }}>Avatar</option>
                                    <option value="badge" {{ old('category') == 'badge' ? 'selected' : '' }}>Insignia</option>
                                    <option value="rank" {{ old('category') == 'rank' ? 'selected' : '' }}>Rango</option>
                                    <option value="skin" {{ old('category') == 'skin' ? 'selected' : '' }}>Tema</option>
                                    <option value="special" {{ old('category') == 'special' ? 'selected' : '' }}>Especial</option>
                                </select>
                                @error('category')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="col-lg-6">
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price', 50) }}" min="0" required>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" class="form-control @error('level_required') is-invalid @enderror" id="level_required" name="level_required" value="{{ old('level_required', 1) }}" min="1" required>
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-switch mb-2">
                                <input type="checkbox" class="custom-control-input" id="is_limited" name="is_limited" {{ old('is_limited') ? 'checked' : '' }}>
                                <label class="custom-control-label" for="is_limited">Item de cantidad limitada</label>
                            </div>
                            <div id="stock-container" class="{{ old('is_limited') ? '' : 'd-none' }}">
                                <label for="stock">Stock disponible</label>
                                <input type="number" class="form-control @error('stock') is-invalid @enderror" id="stock" name="stock" value="{{ old('stock', 10) }}" min="1">
                                @error('stock')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group" id="effects-container">
                            <label for="effects">Efectos (JSON)</label>
                            <textarea class="form-control @error('effects') is-invalid @enderror" id="effects" name="effects" rows="4">{{ old('effects') }}</textarea>
                            @error('effects')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="text-center">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save mr-1"></i> Guardar Item
                    </button>
...
<script>
    $(document).ready(function() {
        ...
        // Inicializar estado del campo de efectos
        const initialCategory = $('#category').val();
        if (initialCategory !== 'special') {
            $('#effects-container').hide();
        }
    });
</script>
